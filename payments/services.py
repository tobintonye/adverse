...
# Webhook Handler — transfer.success / transfer.failed (payout tracking)
def handle_transfer_success(event_data: dict) -> PayoutRecord | None:
    """
    Called when Paystack confirms a payout transfer to an ad manager's bank.
    Marks the PayoutRecord as SUCCESS.
    """
...
def handle_transfer_failed(event_data: dict) -> PayoutRecord | None:
    """
    Called when a payout transfer fails.
    Marks the PayoutRecord as FAILED.
    """
    data = event_data.get("data", {})
    transfer_code = data.get("transfer_code", "")
    ...
    record.mark_failed(gateway_response=_safe_gateway_response(data))
    return record
