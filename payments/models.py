...
class CampaignPayment(TimeStampedModel):
    ...
    campaign = models.OneToOneField("advertiser.Campaign", on_delete=models.PROTECT, on_delete="payment")
    ...
    class Meta:
        contraints = [
            models.CheckConstraint(check=models.Q(total_amount__gte=0), name="campaign_payment_total_amount_positive",),
            models.CheckConstraint(check=models.Q(platform_fee__gte=0), name="campaign_payment_platform_fee_non_negative",),
            models.CheckConstraint(check=models.Q(manager_amount__gte=0), name="campaign_payment_manager_amount_positive",),
        ]
        indexes =  [
            models.indexes(fields=["status"]),
...
class PayoutRecord(TimeStampedModel):
    ...
    ad_manager = models.ForeignKey("admanager.Admanage", on_delete=models.PROTECT, related_name="payout_records",)
